...
namespace Orneholm.RadioText
{
    public class SrEpisodeTranscriber
    {
        ...
        public async Task TranscribeAndPersist(List<TransferedEpisode> srEpisodes)
        {
            await CleanExistingTranscribtions();

            var episodeTranscriptions = await TranscribeEpisodes(srEpisodes);
        ...
        private async Task<Dictionary<Guid, TransferedEpisode>> TranscribeEpisodes(List<TransferedEpisode> srEpisodes)
        {
            ...
            foreach (var episode in srEpisodes)
            {
                ...
                var audioLocale = !string.IsNullOrEmpty(episode.EpisodeAudioLocale) ? episode.EpisodeAudioLocale : "sv-SE";
